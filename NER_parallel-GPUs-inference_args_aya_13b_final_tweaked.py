...
def dataset_upload(lang):
    if not split:
        if lang == "en":
            dataset = load_dataset("bigscience-historical-texts/HIPE2020_sent-split", "en", split="validation",trust_remote_code=True)
        elif lang == "de":
            dataset_val = load_dataset("bigscience-historical-texts/HIPE2020_sent-split", "de", split="validation",trust_remote_code=True)
            dataset_train = load_dataset("bigscience-historical-texts/HIPE2020_sent-split", "de", split="train",trust_remote_code=True)
            dataset = concatenate_datasets([dataset_val, dataset_train])
        elif lang == "fr":
            dataset_val = load_dataset("bigscience-historical-texts/HIPE2020_sent-split", "fr", split="validation",trust_remote_code=True)
            dataset_train = load_dataset("bigscience-historical-texts/HIPE2020_sent-split", "fr", split = "train",trust_remote_code=True)
            dataset = concatenate_datasets([dataset_val, dataset_train])
    else:
        dataset = load_dataset("bigscience-historical-texts/HIPE2020_sent-split", lang, split=split,trust_remote_code=True)
...
def prediction(local_period, period, lang):
    ...
    for sentence_obj in local_period:
        ...
        for ent_type, gold_tokens in entities.items():
            prompt = f"Input: {sentence}\nIn input, what are the {ent2query[ent_type]}? Separate answers with commas without changing the original input text."
